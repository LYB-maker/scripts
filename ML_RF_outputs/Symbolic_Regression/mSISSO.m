...
OUTDIR = "sisso_bandgap_split_out"
if ~exist(OUTDIR, "dir")
mkdir(OUTDIR); end

PLOT_PARAMS.dpi = 600
PLOT_PARAMS.marker_size = 36
PLOT_PARAMS.line_width = 1.2
PLOT_PARAMS.fig_w_in = 6.0
PLOT_PARAMS.fig_h_in = 6.0
...
fprintf(
    "Loaded data_v4.csv: %d samples, %d base features.\n", size(
        X0, 1), size(
            X0, 2))

[X, featureList] = generateDescriptorLibrary(X0, featureList0)
fprintf(
    "Generated descriptor library: %d samples, %d features.\n", size(
        X, 1), size(
            X, 2))
...
nNonzeroCoefs = 4
nFeaturesPerSisIter = 25
fprintf(
    "Searching for models up to %d dimensions, considering %d new features per iteration.\n",
    nNonzeroCoefs,
     nFeaturesPerSisIter)
...
runDat = struct()
runDat.seed = SEED
runDat.featureList0 = featureList0
runDat.featureList = featureList
runDat.idxTrain = idxTrain
runDat.idxVal = idxVal
runDat.idxTest = idxTest
runDat.nNonzeroCoefs = nNonzeroCoefs
runDat.nFeaturesPerSisIter = nFeaturesPerSisIter

runDat.y_tr = ytr; runDat.pred_tr = yhat_tr
runDat.y_va = yva; runDat.pred_va = yhat_va
runDat.y_te = yte; runDat.pred_te = yhat_te

runDat.metrics_train = [r2_tr, mae_tr, rmse_tr]
runDat.metrics_val = [r2_va, mae_va, rmse_va]
runDat.metrics_test = [r2_te, mae_te, rmse_te]

runDat.sisso_coefs = sisso.coefs
runDat.sisso_intercept = sisso.intercept
runDat.sisso_selectedIndices = sisso.selectedIndicesL0{dim}
runDat.sisso_selectedFeatures = featureList(sisso.selectedIndicesL0{dim})
runDat.sisso_rmses = sisso.rmses

save(fullfile(OUTDIR, "sisso_results.mat"), "-struct", "runDat");

write_parity_dat(ytr, yhat_tr, fullfile(OUTDIR, "parity_train.dat"));
write_parity_dat(yva, yhat_va, fullfile(OUTDIR, "parity_val.dat"));
write_parity_dat(yte, yhat_te, fullfile(OUTDIR, "parity_test.dat"));
...
writecell({"Split", "R2", "MAE", "RMSE"}, fullfile(
    OUTDIR, "metrics.dat"), "Delimiter", "\t");
writecell({"Train", r2_tr, mae_tr, rmse_tr}, fullfile(
    OUTDIR, "metrics.dat"), "Delimiter", "\t", "WriteMode", "append");
writecell({"Val", r2_va, mae_va, rmse_va}, fullfile(
    OUTDIR, "metrics.dat"), "Delimiter", "\t", "WriteMode", "append");
writecell({"Test", r2_te, mae_te, rmse_te}, fullfile(
    OUTDIR, "metrics.dat"), "Delimiter", "\t", "WriteMode", "append");

fid = fopen(fullfile(OUTDIR, "summary.txt"), "w");
fprintf(fid, "Seed: %d\n\n", SEED);
fprintf(fid, "Train/Val/Test sizes: %d / %d / %d\n\n", nTrain, nVal, nTest);
fprintf(fid, "SISSO Parameters:\n");
fprintf(fid, "  Number of nonzero coefficients: %d\n", nNonzeroCoefs);
fprintf(fid, "  Features per SIS iteration: %d\n\n", nFeaturesPerSisIter);
fprintf(fid, "Selected features (dim=%d):\n", dim);
for i = 1:
    length(runDat.sisso_selectedFeatures)
    fprintf(fid, "  %s (coef=%.6f)\n", runDat.sisso_selectedFeatures(i), sisso.listOfNonzeroCoefs{dim}(i));
end
fprintf(fid, "Intercept: %.6f\n\n", sisso.listOfIntercepts{dim});
fprintf(fid, "Train: R2=%.6f  MAE=%.6f  RMSE=%.6f\n", r2_tr, mae_tr, rmse_tr);
fprintf(fid, "Val  : R2=%.6f  MAE=%.6f  RMSE=%.6f\n", r2_va, mae_va, rmse_va);
fprintf(
    fid,
    "Test : R2=%.6f  MAE=%.6f  RMSE=%.6f\n\n",
    r2_te,
    mae_te,
     rmse_te);

modelStr = getModelString(sisso, featureList, dim)
fprintf(fid, "Model equation:\n%s\n", modelStr);
fclose(fid)
...
function obj = SissoRegressor(nNonzeroCoefs, nFeaturesPerSisIter, allL0Combinations)
    if nargin < 1 | | isempty(nNonzeroCoefs), nNonzeroCoefs = 1
    end
    if nargin < 2 | | isempty(nFeaturesPerSisIter), nFeaturesPerSisIter = 1
    end
    if nargin < 3 | | isempty(allL0Combinations), allL0Combinations = false
    end
   ...
   warning('off')
    ...
    for iter = 1:
        ...
        if iter == 1
            ...
            [obj.coefsStandardized, obj.selectedIndicesCurrent, rmse] = exhaustiveRegularization(
                obj, x, yCentered, obj.selectedIndicesSis)
        end

        [coefsThisIter, obj.intercept] = getNonstandardizedCoefs(
            obj, obj.coefsStandardized, yMean)
...
function yPred = predictSisso(obj, x, dim)
    if nargin < 3 | | isempty(dim), dim = obj.nNonzeroCoefs
    end
...
function x = cartesianProduct(sets)
    ...
    for i = 1:
        numSets
        thisSet = sort(sets{i});
        if ~isvector(thisSet), error('All inputs must be vectors.')
        end
        if ~isnumeric(thisSet), error('All inputs must be numeric.')
        end
        if ~isequal(thisSet, unique(thisSet)), error(['Input set ', num2str(i), ' contains duplicated elements.'])
        end
...
function make_parity_plot(y, yhat, ttl, savePath, PLOT_PARAMS, colorIndex)
    y = y(: );
    yhat = yhat(: );

    f = figure("Visible", "off", "Units","inches", "Position",[1 1 PLOT_PARAMS.fig_w_in PLOT_PARAMS.fig_h_in]);
    ax = axes(f)
    hold(ax, "on");
    grid(ax, "on");
    box(ax, "on");
    ...
    scatter(ax, y, yhat, PLOT_PARAMS.marker_size, "filled", ...
        "MarkerFaceColor", c, "MarkerEdgeColor", c, "MarkerFaceAlpha", 0.90)
    ...
    mn = mn - pad
    mx = mx + pad
    plot(ax, [mn mx], [mn mx], "-", "LineWidth", PLOT_PARAMS.line_width, "Color", [0 0 0])
    ...
    exportgraphics(f, savePath, "Resolution", PLOT_PARAMS.dpi)
    close(f)
end

function C = tab10()
    C = [0.1216 0.4667 0.7059

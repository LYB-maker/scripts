...
% targetName = "Formation energy per atom";
targetName = "Bandgap"
...
X_tr = X_rest(idxTr_local, : )
y_tr = y_rest(idxTr_local, : )
X_va = X_rest(idxVal_local, : )
y_va = y_rest(idxVal_local, : )
...
gp = run_gptips2_multigene_regression(
    X_tr_z,
    y_tr_z,
    X_va_z,
    y_va_z,
...
bestParetoTag = pick_model_by_pareto(
    gp, X_tr_z, y_tr_z, X_va_z, y_va_z, candTags, GP);
...
[pred_tr_z, pred_va_z, pred_te_z] = predict_with_gp(
    gp, modelTag, X_tr_z, X_va_z, X_te_z);
...
runDat.sigY = sigY
runDat.y_tr = y_tr
runDat.pred_tr = pred_tr
runDat.y_va = y_va
runDat.pred_va = pred_va
runDat.y_te = y_te
runDat.pred_te = pred_te
...
if ~isempty(fh)
    ...
    make_fitness_plot(
    fh, "Fitness (lower=better)", fullfile(
        OUTDIR, "fitness.png"), GP);
end
writecell({"Split", "R2", "MAE", "RMSE"}, fullfile(
    OUTDIR, "metrics.dat"), "Delimiter", "\t");
writecell({"Train", m_tr(1), m_tr(2), m_tr(3)}, fullfile(
    OUTDIR, "metrics.dat"), "Delimiter", "\t", "WriteMode", "append");
writecell({"Val", m_va(1), m_va(2), m_va(3)}, fullfile(
    OUTDIR, "metrics.dat"), "Delimiter", "\t", "WriteMode", "append");
writecell({"Test", m_te(1), m_te(2), m_te(3)}, fullfile(
    OUTDIR, "metrics.dat"), "Delimiter", "\t", "WriteMode", "append");
...
fprintf(
    fid,
    "Train: R2=%.6f  MAE=%.6f  RMSE=%.6f\n",
    m_tr(1),
    m_tr(2),
     m_tr(3));
fprintf(
    fid,
    "Val  : R2=%.6f  MAE=%.6f  RMSE=%.6f\n",
    m_va(1),
    m_va(2),
     m_va(3));
fprintf(
...
if ~any(allVars == targetName)
    ...
    if isempty(cand)
        error(
    "Target column not found: %s. Available vars:\n%s",
    targetName,
    strjoin(
        allVars,
         ", "));
...
if isempty(y) | |~isfinite(nsig) | |nsig <= 0
return
end
m = mean(y, "omitnan");
s = std(y, 0, "omitnan");
...
gp = rungp( @ configFcn);
    function gp = configFcn(gp)
        ...
        if isfield(GP, "timeout_sec") & & GP.timeout_sec > 0
           try
            gp.runcontrol.timeout = GP.timeout_sec
            catch; end
        end
        try
        ...
        try
        gp.selection.tournament.size = GP.tournament_size
        catch; end
        try
        gp.selection.elite_fraction = GP.elite_fraction
        catch; end
        try
        gp.selection.lexicographic = GP.use_lexicographic
        catch; end
        try
        gp.fitness.cache = GP.cache_fitness
        catch; end
        if isfield(GP, "parsimony_on") & & GP.parsimony_on
           try
            gp.selection.tournament.p_pareto = GP.parsimony_p_pareto
            catch; end
            try
            gp.selection.tournament.pareto = GP.parsimony_p_pareto
            catch; end
        end
        ...
        try
        gp.nodes.terminalSet = cellstr(featNames)
        catch; end
        try
...
candidates = {
    "results.bestfithist",
    "results.stats.bestfitness",
    "results.stats.bestfit",
    "state.bestfitness",
    "state.bestfit",
     "results.fithistory"};
for i = 1:
...
c = NaN
candidates = {
    "results.(tag).complexity",
    "results.(tag).comp",
     "results.best.complexity"};
for i = 1:
    numel(candidates)
...
1.0000 0.4980 0.0549;0.1725 0.6275 0.1725;0.8392 0.1529 0.1569;0.5804 0.4039 0.7412;0.5490 0.3373 0.2941;0.8902 0.4667 0.7608;0.4980 0.4980 0.4980;0.7373 0.7412 0.1333;0.0902 0.7451 0.8118]
end

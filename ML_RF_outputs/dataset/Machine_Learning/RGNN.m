...
trainCfg.maxEpoch = 200
trainCfg.batch = 64
trainCfg.lr = 1e-3
trainCfg.wd = 8e-4
trainCfg.gradClip = 1.0
trainCfg.lrDropEvery = 40
trainCfg.lrDropFactor = 0.5
trainCfg.patience = 5
trainCfg.emaDecay = 0.995
trainCfg.ensN = 5
...
for k = 1:
    ...
    for e = 1:
        ...
        [emaNet,
         bestNet,
         bestInfo] = train_dlnet_reg(dlnet,
                                     XTrainN,
                                     YTrainN,
                                     XValN,
                                     YValN,
...
[limParity] = compute_parity_lim_all(
    YTrain_all,
    YHatTrain_all,
    YVal_all,
    YHatVal_all,
...
function[emaNet, bestNet, info] = train_dlnet_reg(
    dlnet, XTrain, YTrain, XVal, YVal, cfg, hwb, k, K, e, ensN, stepBase, totalSteps)
mb = cfg.batch
...
for epoch = 1:
    ...
    msg = sprintf("Fold %d/%d | ensN %d/%d | Epoch %d/%d",
                  k, K, e, ensN, epoch, cfg.maxEpoch)
    ...
    for it = 1:
        ...
        [dlnet, trailingAvg, trailingAvgSq] = adamupdate(
            dlnet, grads, trailingAvg, trailingAvgSq, epoch * 100000 + it, lr)

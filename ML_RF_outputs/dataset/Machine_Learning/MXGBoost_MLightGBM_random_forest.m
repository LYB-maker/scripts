...
for mi = 1:
    ...
    for k = 1:
        ...
        if isvalid(hwb)
            waitbar(min(1, (k - 1) / totalSteps), hwb,
                    sprintf("Fold %d/%d | %s", k, K, modelName))
            drawnow limitrate
        end
        ...
        if k == 1
           rng(777 + 10 * mi)
            ...
            for tr = 1:
                tuneCfg.nTrials
                [mdl, info, yhatValN, yhatTrSubN] = train_one_trial(
                    modelName, XTrainN, YTrainN, XValN, YValN, XTrainSub, YTrainSub, d, mi, k, tr);
